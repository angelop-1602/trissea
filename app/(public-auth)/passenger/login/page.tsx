...
export default function PassengerLoginPage() {
  ...
  const sendOtp = async (event: FormEvent) => {
    ...
    if (!canSendOtp) {
      setError('Enter a valid mobile number before requesting OTP.');
      return;
    }
  ...
  const verifyOtp = async (event: FormEvent) => {
    ...
    try {
      ...
      if (mePayload.user.role !== 'passenger') {
        await fetch('/api/auth/logout', { method: 'POST' });
        setError('This mobile number is not linked to a passenger account.');
        setOtpSent(false);
        setToken('');
        return;
      }
  ...
  return (
    <MobileAuthShell
      ...
      subtitle="Sign in with your mobile number and one-time passcode."
      mode="login"
      tone="passenger"
      backHref="/passenger"
    ...
    >
      {!otpSent ? (
        <form className="space-y-4" onSubmit={sendOtp}>
          <AuthInput
            icon={Phone}
            type="tel"
            placeholder="Mobile number (e.g. +639171234567)"
            value={phone}
      ...
      <div className="space-y-1 text-center text-sm text-muted-foreground">
        <p>
          ...
          <Link href="/passenger/signup" className="font-semibold text-foreground hover:text-primary">
            Sign up
          </Link>
        </p>
      </div>
